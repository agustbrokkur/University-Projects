...
CREATE OR REPLACE VIEW FS1 (PID, totalBalances)
AS 
...
CREATE OR REPLACE VIEW FS2 (PID, totalUnPaid)
AS 
...
CREATE OR REPLACE FUNCTION PayOneBill( IN iBID INTEGER ) 
RETURNS VOID 
AS $$
DECLARE
...
------------------------------------------------------------------------------------------------------------
-- 11. LoanMoney (Procedure)
------------------------------------------------------------------------------------------------------------

CREATE OR REPLACE FUNCTION PayAllBills(IN iPID INT)
RETURNS void
AS $$
DECLARE 
	-- First, make a record ready (will adjust to any table)
	tBill   RECORD;
		
BEGIN
	-- Bind tBill to a FOREACH type of loop:
	FOR tBill IN
		SELECT *
		FROM Bills B
		WHERE 	B.PID = iPID 
		AND B.bDueDate < CURRENT_DATE 
		AND B.bIsPaid = false
	LOOP
		-- perform payment on selected bill
		-- Notice that PERFORM must be used instead of SELECT if executed from a function 
		-- (so postgres can dump the returned data...)
		PERFORM payOneBill(tBill.BID);
		RAISE NOTICE 'BIll paid';
	END LOOP;
	END; 
$$
LANGUAGE plpgsql;
