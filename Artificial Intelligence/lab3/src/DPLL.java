...
public class DPLL implements InferenceProcedure {
	...
	public boolean isEntailed(KnowledgeBase kb, Sentence alpha) {
		...
		boolean dple = !dpll(kbAndNotAlpha, new ArrayList<>(symbols), new Model());
		
		long elapsedTime = System.currentTimeMillis() - startTime;
		System.out.println("The query is: " + notQuery + ", The time is: " + elapsedTime + "ms" + ", The number is: " + x);
	...
	public boolean dpll(Set<Clause> clauses, List<PropositionSymbol> symbols, Model model) {
		...
		Pair<PropositionSymbol, Boolean> findPure = findPureSymbol(symbols, clauses, model);
		if (findPure != null) {
			symbols.remove(findPure.getFirst());
			return callDPLL(clauses, symbols, model, findPure.getFirst(), findPure.getSecond());
		}
		
		Pair<PropositionSymbol, Boolean> findUnit = findUnitClause(clauses, model);
		if(findUnit != null) {
			symbols.remove(findUnit.getFirst());
			return callDPLL(clauses, symbols, model, findUnit.getFirst(), findUnit.getSecond());
